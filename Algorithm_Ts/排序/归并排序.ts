function guibingSort(arr: Array<number>): Array<number> {
    if (arr.length <= 1) return arr;
    // 递归
    const mid = Math.floor(arr.length / 2);
    const leftArr = guibingSort(arr.slice(0, mid)); // 返回有序的left子数组
    const endArr = guibingSort(arr.slice(mid)); // 返回有序的right子数组
    // 核心代码，合并代价
...
/* 归并排序 */
const arr = [6, 9, 3, 8, 0, -1, 10];
const newarr = guibingSort(arr);
console.log(newarr);